...
object SignatureVerifier {
    ...
    // App A (MederPayEnforcerA) expected signature
    ...
    fun verifyCompanionSignature(context: Context, packageName: String): Boolean {
        try {
            ...
            val matchesRelease = constantTimeCompare(actualSignatureHash, EXPECTED_APP_A_SIGNATURE)
            val matchesDebug = constantTimeCompare(actualSignatureHash, EXPECTED_APP_A_SIGNATURE_DEBUG)
            
            val isValid = matchesRelease || matchesDebug
            
            if (!isValid) {
                Log.e(TAG, "Signature mismatch for $packageName")
                Log.e(TAG, "Expected: $EXPECTED_APP_A_SIGNATURE or $EXPECTED_APP_A_SIGNATURE_DEBUG")
    ...
    fun areSignaturesConfigured(): Boolean {
        return !EXPECTED_APP_A_SIGNATURE.startsWith("PLACEHOLDER") &&
               !EXPECTED_APP_A_SIGNATURE_DEBUG.startsWith("PLACEHOLDER")
    }
}
