...
APP_A_DIR="$PROJECT_ROOT/MederPayEnforcerA"
APP_B_DIR="$PROJECT_ROOT/MederPayEnforcerB"
BUILD_TYPE="${1:-release}"  # Default to release, can override with 'debug'

# Validate build type
if [[ "$BUILD_TYPE" != "release" && "$BUILD_TYPE" != "debug" ]]; then
    echo "Error: Build type must be 'release' or 'debug'"
    echo "Usage: ./build-dual-apps.sh [release|debug]"
    exit 1
fi
...
echo "2. Test on physical device(s)"
